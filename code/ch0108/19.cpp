 ...
 *
 * 描述
 *     一张CT扫描的灰度图像可以用一个N*N（0 < N <= 100）的矩阵描述，矩阵上的每个点对应一个灰度值（整数），其取值范围是0-255。我们假设给定的图像中有且只有一个肿瘤。
 *      在图上监测肿瘤的方法如下：
...
int main()
{
    ...
    for (i = 1; i <= n; i++){
        for (j = 1; j <= n; j++){
            if(a[i][j]<=50&&(i==1||j==1||i==n||j==n||a[i-1][j]>50||a[i+1][j]>50||a[i][j-1]>50||a[i][j+1]>50)){
                len++;
            }
        }
    }
